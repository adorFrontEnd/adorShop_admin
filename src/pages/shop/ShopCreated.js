...
class Page extends Component {
  ...
  componentDidMount() {
    ...
    shopOperData = shopOperData ? JSON.parse(shopOperData) : null;
    this.setState({ shopOperData, _title, idstatus: id, isChange });
  }
  ...
  saveDataClicked = () => {
    this.props.form.validateFields((err, data) => {
      ...
      let { date, imageUrl, shopOper, categoryIds, shopOperData, status, idstatus } = this.state;
      if (!status && idstatus == 0) {
        Toast('请先检测手机号');
        return;
      }
      let deadlineStamp = dateUtil.getDayStartStamp(Date.parse(date));
      let id = shopOperData ? shopOperData.id : null;
  ...
  render() {
    ...
    const { shopOper, category, shopOperData } = this.state
    return (
      <CommonPage title={this.state._title} description={_description} style={{ padding: '0' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row-reverse' }}>
          <Button className='save-btn' type='primary' onClick={this.saveDataClicked} disabled={this.state.status == 0}>保存</Button>
