...
class CategorySelection extends Component {
  ...
  _getAllAuthList = () => {
    ...
    searchList()
      .then(data => {
        //   console.log(data)
        let allAuth = data.data
        let normalAuthSortMap = this.getSourceSortNumMap(allAuth);
        let allSpecAuth = data.data.filter(item => item.level == 2);
        let allAuthList = allAuth.map(item => item.name);
  ...
  selectRole = (id) => {
    ...
    if (!id) {
      ...
      arr = this.state.categoryList.filter(item => item.id == id);
    }
    ...
    if (this.state.canRenderTree && this.state.authTree && this.state.allSpecAuthList) {
      return (
        <div>
          ...
          >
            <TreeNode icon={<Icon type="deployment-unit" />} title="全部" key="all">
              {
...
export default Form.create()(CategorySelection);
